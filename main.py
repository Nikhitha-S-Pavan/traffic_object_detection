...
class Page(tk.Frame):
    def __init__(self, parent, window):
        ...
        menuFrame.grid(row=3, column=0, sticky="NSW", padx=5, pady=2)
        # Button that lets the user take a snapshot
        self.btnSaveImage = tk.Button(
            menuFrame, text="Save Image", command=self.saveImage
        )
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs) > 0:
                        ...
                        tk.Label(
                            self.window, text="Count - " + str(counter), borderwidth=1
                        ).grid(row=1, column=0)
